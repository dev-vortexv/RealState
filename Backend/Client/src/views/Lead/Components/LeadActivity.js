...
export default function EmailGrid(props) {
  ...
  return (
    <Box>
      ...
      <CallDialog open={openCallDialog} onClose={handleCloseCallDialog} recipientName={emailsToShow[0]?.createByName} />
      <SendMailDialog open={openSendMailDialog} onClose={handleCloseSendMailDialog} recipientName={emailsToShow[0]?.createByName} />
                        ...
                        <TableBody>
                          {meetingToShow?.slice(0, showMoreMeetings ? meetingToShow.length : 1).map((meetings, index) => (
                            <TableRow key={index}>
                              ...
                              <TableCell>{moment(meetingToShow.startdate).format('hh:mmA (MM/DD)')}</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                      </Table>
